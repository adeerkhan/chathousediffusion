...
def feature_to_mask(feature):
    ...
    mask = torch.ones_like(contour)
    mask = mask.where(contour > 0.9, 0)
    return mask
...
class Unet(nn.Module):
    ...
    def forward(self, x, time, x_self_cond=None, feature=None):
        assert all(
...
class GaussianDiffusion(nn.Module):
    ...
    def p_losses(self, x_start, feature, t, noise=None, offset_noise_strength=None):
        ...
        model_out = self.model(x, t, x_self_cond, feature)
